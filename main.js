...
// JavaScript for volume control
const audio = new Audio();  // Create an audio object
audio.src = 'path_to_your_audio_file.mp3'; // Specify your audio source here

// Function to change the volume
function change_volume() {
  const volumeSlider = document.getElementById("volume_slider");
  const volume = volumeSlider.value / 100; // Volume is from 0 to 1
  audio.volume = volume; // Set the audio volume
}

// Function to play/pause the audio (just for completeness)
function togglePlayPause() {
  const playPauseBtn = document.getElementById("play_pause_btn");
  if (audio.paused) {
    audio.play();
    playPauseBtn.innerHTML = "<i class='fa fa-pause'></i>"; // Change button to pause
  } else {
    audio.pause();
    playPauseBtn.innerHTML = "<i class='fa fa-play'></i>"; // Change button to play
  }
}
